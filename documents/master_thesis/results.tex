...
\end{itemize}
To answer these questions I have run several experiments. The ones related to the first question can be found in \secref{comparison_algorithms}. To answer the second question I will describe in \secref{large_scale_powernets} the results of running \emph{HELM} on a large scale power net with a few thousand nodes.
...
To compare the different load-flow algorithms I used the sample nets of the Institute and compared the algorithms then regarding their runtime and accuracy. I have already considered the improved convergence behaviour in \secref{implementation_helm}, but I will show additional results regarding this aspect in this chapter. The parameters for the algorithms in the accuracy and runtime comparison can be found in \tabref{comparison_parameter}, the settings for the convergence border tests are in \tabref{comparison2_parameter}.
...
First, I would like to point out the relative power errors of the algorithms for these two versions of the power net in \figref{big_net_separate_relative_power_error} and \figref{big_net_combined_relative_power_error}. For most applications of a load-flow algorithm this accuracy should be sufficient.
