...
To solve these linear equation systems for instance with a LU-factorization is for big power quite slow. The solution to this problem is to use iterative solvers like \emph{BiCGSTAB} \cite{bicgstab}.
...
The application of the special conditions in \emph{HELM} allows a few optimization, which can not be made in general. At the beginning I used \emph{Eigen} \footnote{http://eigen.tuxfamily.org/} for the linear algebra, but with bigger problems I ran into performance problems with this general purpose library. Consequently, I reimplemented the necessary data structures and algorithms:
\begin{itemize}
	\item Dense vector
	\item Sparse matrix
...
However, the system matrix of the linear equation system is the admittance matrix, which is dense for big problems. In fact, the amount of nonzero values in every row is nearly independent of the size of the total matrix. The reason for this lies within the physical structure of a power net, where every node only as a limited number of neighbours, no matter how big the total graph is. Therefore, the density of the admittance matrix decreases with a growing problem size. For small problems this specialisation is in fact a performance drawback, but for these problems the performance does not matter anyway. As there must not be a zero-row the LIL \cite{sparseMatricesParallel} version is superior over CRS \cite{sparseMatricesJava} or CCS \cite{sparseMatricesJava}.

The multiplication of a sparse matrix with a dense vector can then leverage these two special formats. The key point is to iterate only over the nonzero elements over each row in the matrix and select the according elements of the vector in $\mathcal O(1)$. Additionally, the operation can be parallelized effectively, as all elements in the result vector are independent from each other.

Operations like the dot product or adding two dense vector can be parallelized too. In this case the dense storage of the values is again very convenient, regarding the performance of the operations.
