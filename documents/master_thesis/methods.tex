...
\begin{equation}
	...
	\end{split}
\end{equation}
I can now differentiate these formulas with respect to $U_k$, $U_i$ ($i \ne k$), $\delta_k$ and $\delta_i$ ($i \ne k$):
\begin{equation}
	\begin{split}
...
With these derivations I can calculate the derivation of the power functions, which form the Jacobian matrix
\begin{equation}
	\mat S_1' (\vec x^{(k)}) = 
	\begin{bmatrix}
...
\end{equation}
In this equation I have now an additional parameter $s$, where a selection of $s = 1$ would result in the original formulation. Consequently, the target is to evalute the functions $U_i(s)$ at the point $s = 1$.
...
At this point now, all necessary formulas are available to be able to calculate the coefficients up to a certain index. The next step is then the analytic continuation, which evaluates the voltage functions at a certain point.
