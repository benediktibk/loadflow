...
A feed-in \figref{feedin} is characterized by the voltage at the internal slack bus $U_{slack}$, the short circuit power $S_k$, the power factor $c$ and the ratio of the real to the imaginary part $\frac{R}{X}$. From these values the magnitude of the input impedance 
\begin{equation}
	|Z_q| = c \frac{U_n}{S_k}
\end{equation}
...
As \emph{HELM} is a fairly new algorithm and only little information is publicly available, I shall add a short numerical example in \chapinref{helm_example}. I will also include the coefficients and steps of \emph{Wynn's Epsilon algorithm} to make it easier to reimplement \emph{HELM}.