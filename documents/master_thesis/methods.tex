...
\end{equation}
as the coefficients $d_{i,j}$ depend only on the previous $c_{i,j}$. All other coefficients are then the solution of
\begin{equation}
...
\begin{equation}
	|U_k|^2 \sum_i Y_{ki} U_i = s 2 P_k U_k - s U_k^2 \sum_i Y_{ki}^\star U_i^\star + s U_k^2 I_k^\star + |U_k|^2 I_k + (1 - s) |U_k|^2 Y_k,
\end{equation}
which is after the division by $|U_k|^2$
\begin{equation}
	\sum_i Y_{ki} U_i = s \frac{2 P_k}{|U_k|^2} U_k - s \frac{1}{|U_k|^2} U_k^2 \sum_i Y_{ki}^\star U_i^\star + s U_k^2 \frac{I_k^\star}{|U_k|^2} + I_k + (1 - s) Y_k
...
\begin{equation}
	F_{PV}(s) = s \frac{2 P_k}{|U_k|^2} U_k - s \frac{1}{|U_k|^2} U_k^2 \sum_i Y_{ki}^\star U_i^\star + s U_k^2 \frac{I_k^\star}{|U_k|^2} + I_k + (1 - s) Y_k.
	\label{eq:rhs_pv}
\end{equation}
...
\begin{equation}
	F_{PV}(s) = s \frac{2 P_k}{|U_k|^2} U_k(s) - s \frac{1}{|U_k|^2} X_k(s) - s U_k(s) + s \frac{I_k^\star}{|U_k|^2} V_k(s) + I_k + (1 - s) Y_k
\end{equation}
...
\begin{enumerate}
	\item Calculate the first coefficients with
	\begin{equation}
		\sum_i Y_{ki} c_{i,0} = 
		\begin{cases}
			Y_k & \text{if k is a PQ-bus} \\
			I_k + Y_k & \text{if k is a PV-bus}
		\end{cases}
	\end{equation}
	\item Calculate the second coefficients with
	\begin{equation}
		\sum_i Y_{ki} c_{i,0} = 
		\begin{cases}
			I_k + S_k^\star d_{k,0} - Y_k & \text{if k is a PQ-bus} \\
			\frac{2 P_k}{|U_k|^2} c_{k,0} - \frac{1}{|U_k|^2} x_{k,0} - c_{k,0} + \frac{I_k^\star}{|U_k|^2} v_{k,0} - Y_k & \text{if k is a PV-bus}
		\end{cases}
	\end{equation}
	\item Calculate the other coefficients with
	\begin{equation}
		\sum_i Y_{ki} c_{i,n} = 
		\begin{cases}
			S_k^\star d_{k,j - 1} & \text{if k is a PQ-bus} \\
			\frac{2 P_k}{|U_k|^2} c_{k,n-1} - \frac{1}{|U_k|^2} x_{k,n-1} - c_{k,n-1} + \frac{I_k^\star}{|U_k|^2} v_{k,n-1} & \text{if k is a PV-bus}
		\end{cases}
	\end{equation}
	\item Evaluate the voltage functions $U_i(s = 1)$ with \eqref{epsilon_wynn}
\end{enumerate}

The derived coefficients $d_{k,j}$ during the first step can be calculated with
\begin{equation}
	d_{i,0} = \frac{1}{c_{i,0}^\star},
\end{equation}
