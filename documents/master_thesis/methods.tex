...
\begin{enumerate}
	\item Calculate the current power mismatch $\Delta \vec S (\vec x_k)$ with \neweqref{power_mismatch}
	\item Calculate the Jacobian matrix $\vec S' (\vec x_k)$ with \neweqref{power_jacobian_newton_raphson}
	\item Calculate the voltage changes $\Delta \vec x_k$ through solving the linear equation system \neweqref{newton_raphson_equation_system}
...
With this simplified system matrix the equation system \neweqref{newton_raphson_equation_system} can be seperated into
\begin{equation}
