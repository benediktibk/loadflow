...
\begin{equation}	
		\sum_j Y_{ij} U_{j}(s) = s I_j + \frac{s S_j^\star}{U_j^\star(s^\star)} + (1 - s) Y_i.
...
\begin{equation}
		\sum_j \left( Y_{ij} \sum_{k = 0}^\infty c_{j,k} s^k \right) = s I_j + \frac{s S_j^\star}{\sum_{k = 0}^\infty c_{j,k}^\star s^k} + (1 - s) Y_i.
